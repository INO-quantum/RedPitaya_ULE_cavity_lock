...
The offset of the laser from the TEM00 resonance of the ULE cavity is adjusted by a fiber-coupled EOM (electro-optic modulator, Jenoptic PM1064). The offset frequency of order 500MHz (-5dBm to 0dBm depending on frequency) is generated by an independent function generator (frequency is manually adjusted). 

The PDH scheme needs an additional modulation frequency (3MHz, +4dBm) which is generated by the RedPitaya board and mixed with the offset frequency (minicircuits ZFM-150+, `LO`=modulation, `RF`=offset frequency), see image below. The mixed signal (about -12dBm) is amplified (minicircuits ZHL-1-2W, 33dB gain gives about +21dBm output) and sent to the EOM. We are actually using the ±2nd order sidebands of the EOM to lock the laser (see table below for details).

RF signal after the mixer and before amplification for Yb-174 at 492MHz offset frequency:
