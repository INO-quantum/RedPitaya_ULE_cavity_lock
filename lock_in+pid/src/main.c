...
//#include "rp-hw.h" // added by Andi for LEDs
#include "rp_hw.h" // in OS-2.0 changed name!

/* Describe app. parameters with some info/limitations */
// Andi: these are the parameters as sent by app and not the register values!
//       name, value, fpga_update, read_only, min, max
...
static rp_app_params_t rp_main_params[PARAMS_NUM+1] = {
  ...
  //{ "lock_gen_mod_phase_sq"         ,      0, 1, 0,            0,   0xffffffff }, /** phase relation of sq_phas signal **/
    { "lock_gen_mod_phase_sq"         ,      0, 1, 0,         -360,          360 }, /** rel. phase in degree of sq_phas signal. modified by Andi **/
    { "lock_gen_mod_hp"               ,      0, 1, 0,            0,        16383 }, /** harmonic period set **/
  //{ "lock_gen_mod_sqp"              ,      0, 1, 0,            0,   0xffffffff }, /** square signal period **/
    { "lock_gen_mod_sqp"              ,      0, 1, 0,            0,       62.5e6 }, /** square signal frequency in Hz. modified by Andi **/
    ...
    { "lock_gen_mod_sqp2"             ,      0, 1, 0,            0,       62.5e6 }, /** square signal frequency in Hz added by Andi **/
    { "led_state"                     ,      0, 0, 0,            0,            7 }, /** LED state, added by Andi **/
...
// Andi 2/12/2025: switch off LEDs to avoid interference
// requires librp-hw.so to be linked in Makefile!
// flags = combination of: 1 = orange (memory card), 2 = red (heartbeat), 4 = ethernet
int enable_LEDs(int flags) {
    int error = 0, tmp;
    tmp  = rp_SetLEDMMCState(flags & 1);
    if (!error) error = tmp;
    tmp = rp_SetLEDHeartBeatState(flags & 2);
    if (!error) error = tmp;
    tmp = rp_SetLEDEthState(flags & 4);
    if (!error) error = tmp;
    if (error) fprintf(stderr, "Andi: switch LEDs state %d error %d\n", flags, error);
    else       fprintf(stderr, "Andi: switch LEDs state %d ok\n", flags);
    return error;
}

const char *rp_app_desc(void)
{
    return (const char *)"Red Pitaya osciloscope+lock-in+PID application. (modified by Andi)\n";
}
...
int rp_app_init(void)
{
    ...
    // added by Andi to disable LEDs by default
    enable_LEDs(0);
...
int rp_set_params(rp_app_params_t *p, int len)
{
    ...
    for(i = 0; i < len || p[i].name != NULL; i++) {
        ...
        if(rp_main_params[p_idx].value != p[i].value) {
            ...
            if ( (p_idx >= PARAMS_PID_PARAMS) && (p_idx < PARAMS_LOCK_PARAMS ) )
                pid_params_change = 1;
            if ( (p_idx >= PARAMS_LOCK_PARAMS) && (p_idx < PARAMS_OTHER ) )
                lock_params_change = 1;
            if(p_idx >= PARAMS_OTHER)
                other_params_change = 1;
    ...
    if (other_params_change) {
        // enable or disable LEDs
        enable_LEDs(rp_main_params[LED_STATE].value);
    }
