...
/*----------------------------------------------------------------------------------*/
//inline int rp_osc_adc_sign(int in_data) // changed from inline by Andi, otherwise is not in controller.so archive!
...
float rp_osc_meas_cnv_cnt(float data, float adc_max_v) // changed from inline by Andi, otherwise is not in controller.so archive!
{
    if(rp_osc_params[LOCK_OSC_RAW_MODE].value>0){ // LOLO
        return data ;
    }else{
