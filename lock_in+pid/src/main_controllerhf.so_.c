...
#include "./rp_hw/rp_hw.h" // added by Andi to switch off LEDs
...
int rp_app_init(void)
{
    ...
    // Andi 1/12/2025: switch off heartbeat led to avoid interference
    // this is declared in rp_hw.h
    rp_SetLEDHeartBeatState(false);     // red heartbeat led
    rp_SetLEDMMCState      (false);     // yellow leds (already off from my firmare)
    //rp_SetLEDEthState      (false);     // Ethernet leds. leave them on since otherwise hard to debug connection problems.
