...
// Andi added constants from parameters defined in main.h
const double lock_gen_mod_fres = LOCK_GEN_MOD_FRES;
const double lock_gen_mod_pres = LOCK_GEN_MOD_PRES;
...
/*----------------------------------------------------------------------------------*/
/**
 * @brief Update LOCK Controller module towards actual settings. Andi: this writes into FPGA registers.
 *
...
int lock_update(rp_app_params_t *params)
{
  ...
  // [FPGAUPDATE DOCK]
    ...
    g_lock_reg->lock_control              = (int) (
                                           ...
                                           ((int)params[LOCK_CTRL_AUX_TRIG_TYPE       ].value)   *   256  + // Andi: allowed values 0..3
  ...
  //g_lock_reg->gen_mod_phase_sq          = (int)params[LOCK_GEN_MOD_PHASE_SQ         ].value;
    g_lock_reg->gen_mod_phase_sq          = ((int32_t)(((double)params[LOCK_GEN_MOD_PHASE_SQ].value)/lock_gen_mod_pres)); // % (1<<14); // modified by Andi. convert +/-degree to register.
    g_lock_reg->gen_mod_hp                = (int)params[LOCK_GEN_MOD_HP               ].value;
  //g_lock_reg->gen_mod_sqp               = (int)params[LOCK_GEN_MOD_SQP              ].value;
    g_lock_reg->gen_mod_sqp               = (uint32_t)(((double)params[LOCK_GEN_MOD_SQP].value)/lock_gen_mod_fres); // modified by Andi. convert Hz to register.
  ...
  //g_lock_reg->ctrl_B                    = (int)params[LOCK_CTRL_B                   ].value;
    ...
    g_lock_reg->gen_mod_sqp2              = (uint32_t)(((double)params[LOCK_GEN_MOD_SQP2].value)/lock_gen_mod_fres); // added by Andi: convert Hz to register
    fprintf(stderr, "Andi: lock_update reg0 = %u <- value = %f, res = %f\n", g_lock_reg->gen_mod_sqp , params[LOCK_GEN_MOD_SQP ].value, lock_gen_mod_fres);
    fprintf(stderr, "Andi: lock_update reg1 = %u <- value = %f, res = %f\n", g_lock_reg->gen_mod_sqp2, params[LOCK_GEN_MOD_SQP2].value, lock_gen_mod_fres);
    fprintf(stderr, "Andi: lock_update reg3 = %u <- value = %f, res = %f\n", g_lock_reg->gen_mod_phase_sq, params[LOCK_GEN_MOD_PHASE_SQ].value, lock_gen_mod_pres);
  // [FPGAUPDATE DOCK END]
...
/*----------------------------------------------------------------------------------*/
/**
 * @brief Update LOCK Controller module towards actual settings FROM THE FPGA REGs. Andi: this reads from FPGA registers.
 *
...
int lock_update_main(rp_app_params_t *params)
{
    ...
    params[LOCK_GEN_MOD_PHASE_SQ].value = ((double)g_lock_reg->gen_mod_phase_sq)*lock_gen_mod_pres; // lock_gen_mod_phase_sq // modified by Andi. convert register to degree
    ...
    params[LOCK_GEN_MOD_SQP].value = ((double)g_lock_reg->gen_mod_sqp)*lock_gen_mod_fres; // lock_gen_mod_sqp // updated by Andi. convert register to Hz
    ...
    params[LOCK_GEN_MOD_SQP2].value = ((double)g_lock_reg->gen_mod_sqp2)*lock_gen_mod_fres; // lock_gen_mod_sqp2 added by Andi. convert register to Hz
    //fprintf(stderr, "Andi: reg = %u -> value = %f, res = %f\n", g_lock_reg->gen_mod_sqp2, params[LOCK_GEN_MOD_SQP2].value, lock_gen_mod_fres);
